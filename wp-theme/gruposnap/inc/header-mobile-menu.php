<?php
/**
 * Menú móvil Printme — logo arriba y pie (idiomas, accesibilidad, CTAs).
 *
 * @package Gruposnap
 */
...
function gruposnap_mobile_menu_icon_svg(string $icon): string
{
    switch ($icon) {
        ...
        case 'a11y':
            return '<svg viewBox="0 0 24 24" width="20" height="20" aria-hidden="true" focusable="false"><circle cx="12" cy="4" r="2" fill="currentColor"/><path fill="currentColor" d="M3 8h18v2l-6 1v3l2 7h-2l-2-6h-2l-2 6H7l2-7v-3L3 10z"/></svg>';
        default:
            return '';
    }
}
...
function gruposnap_render_mobile_menu_extras_template(): void
{
    ...
    $home_url  = function_exists('gruposnap_lang_url') ? esc_url(gruposnap_lang_url('/')) : esc_url(home_url('/'));
    $whatsapp  = esc_url(gruposnap_mobile_menu_whatsapp_url());
    $catalog_url = function_exists('gruposnap_catalog_url') ? gruposnap_catalog_url() : 'https://www.flipsnack.com/gruposnap/cat-logo-grupo-snap/full-view.html';
    $quote_url   = function_exists('gruposnap_header_quote_url') ? gruposnap_header_quote_url() : 'https://wa.link/dutbmb';
    ?>
    <div id="gruposnap-mobile-menu-template" class="gruposnap-mobile-menu__template" hidden>
        ...
        <div class="gruposnap-mobile-menu__footer">
            ...
            <div class="gruposnap-mobile-menu__actions">
                ...
                <a class="gruposnap-mobile-menu__action gruposnap-mobile-menu__action--catalog" href="<?php echo esc_url($catalog_url); ?>" target="_blank" rel="noopener noreferrer">
                    <?php esc_html_e('Catálogo 2026', 'gruposnap'); ?>
                </a>
                <a class="gruposnap-mobile-menu__action gruposnap-mobile-menu__action--quote" href="<?php echo esc_url($quote_url); ?>" target="_blank" rel="noopener noreferrer">
